				...
				<div class="searchbox">
					<script>
						function submitMapForm() {
							var html = '<input type="hidden" name="type" value="map">';
							$('.nearmeform').append(html);
							$('.nearmeform').submit();
						}
					</script>
					<a href="<?php echo base_url('houseshistory/'.$pagelink.'?'.str_replace("&type=map","",$_SERVER['QUERY_STRING'])); ?>"class="btn btn-primary btn-sm"><i class="fa fa-list-ol"></i> </a>
					<a href="<?php echo base_url('houseshistory/'.$pagelink.'?'.$_SERVER['QUERY_STRING']); ?>" class="btn btn-primary btn-sm"><i class="fa fa-map-marker"></i> </a>
